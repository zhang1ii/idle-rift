...
func _run_tests() -> void:
	...
	assert(is_equal_approx(FuryRules.remaining_bleed_burst_damage(10.0, 4, false), 0.0))
	assert(is_equal_approx(FuryRules.remaining_bleed_burst_damage(10.0, 4, true), 16.0))
	...
	assert(game.set_talent_enabled(FuryRules.CRIMSON_EXECUTION_TALENT_ID, true))
	...
	game._apply_bleed(0.18)
	var ticks_before: int = game.bleed_ticks_remaining
	var health_before_spender: float = game.enemy_health
	...
	var expected_burst: float = FuryRules.remaining_bleed_burst_damage(
		game.bleed_tick_damage,
		game.bleed_ticks_remaining,
		true,
	)
	game._cast_fury_skill(
		"single_spender",
	...
	assert(is_equal_approx(
		health_before_spender - game.enemy_health,
		base_spender_damage + expected_burst,
	))
	assert(game.bleed_ticks_remaining == ticks_before)
	print("Blood talent tests passed: full Blood branch combat effects.")
	quit()
