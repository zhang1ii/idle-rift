...
## 14. 2026-07-21 第一裂隙正式楼层身份

把 1～10 层从通用怪物轮换推进为数据驱动的正式内容：

- `first_rift.json` 现在包含完整 10 层定义、区域名、普通敌人轮换、机制标签和教学目标。
- 第 5 层正式显示为“裂隙守关者·碎岩”，第 10 层显示为“逆序守关者·刻轮”。
- 战斗顶部会显示当前普通层或 Boss 层的机制标签。
- 第 2 层腐血真实叠加至 3 层，并按敌人攻击力持续造成战损。
- 第 3 层厚甲使直接伤害降低 25%，流血 DOT 可以穿透。
- 第 4 层每第三次攻击固定变为 225% 裂地重击，状态栏提前预警。
- 第 6～9 层使用独立区域和敌人名称，并继续承接已有的武器/精通/全能/急速定向掉落。
- 第 5/10 层 Boss 时间轴和已经校准的生命、伤害、掉落率没有改动。

新增 `tests/test_rift_floor_content.gd`，并把第一裂隙推进测试扩展到第 10 层和第二个 Boss 天赋点。

详细表格和参数见 `docs/RIFT_FLOORS_1_10.md`。普通层 20 杀逐层解锁目前仍只存在于规则模型，下一步应接入完整战斗界面。

## 15. 2026-07-23 试玩版表现收敛

本轮目标从继续扩规则改为先让外部玩家稳定完成一轮试玩：

- 我方攻击不再跨越半个战场，普通攻击踏步限制为 16 像素，强攻击限制为 24 像素，并在动作结束后回到固定站位。
- 新动作会取消旧移动和颜色 Tween，敌人刷新、受击、死亡与连续技能不再互相叠加。
- 敌人刷新位移限制为 8 像素，死亡倾斜与颜色会在下一场战斗前清零。
- 同屏伤害数字上限为 4，命中与护盾特效均有独立生命周期和统一重置入口。
- 返回战前、开始新战斗和地板坍塌时都会重置全部表现状态。
- 当前实际使用的狂怒战士与裂隙野猪动画已纳入运行时资源清单。
- 新增 Windows 一键启动入口 `PLAYTEST_WINDOWS.bat` 和 5～10 分钟试玩路线 `docs/PLAYTEST_GUIDE.md`。

下一步先用外部试玩反馈确认“规则是否看得懂、失败原因是否明确、入口是否找得到”，再接普通层 20 杀解锁和存档。
