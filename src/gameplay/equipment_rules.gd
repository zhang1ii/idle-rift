...
const SET_DEFINITIONS := {
	...
	"iron_vow": {
		"name": "铁誓战甲",
		"affixes": ["versatility", "critical_strike"],
		"bonuses": {
			2: "怒意壁垒转化率提高 10%",
			4: "鲜血回响治疗提高 20%",
			5: "Boss 重击伤害降低 12%",
...
static func create_normal_item(
	...
	if quality != "common":
		...
		var first_index := rng.randi_range(0, available.size() - 1)
		var first_affix: String = available[first_index]
		available.remove_at(first_index)
